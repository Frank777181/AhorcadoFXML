...
public class PalabrasController implements Initializable {
	...
	private void onAñadirButton(ActionEvent event) {
		...
		dialogo.setContentText("La pa palabra debe contener minimo 5 caracteres");
		
		Optional<String> palabra = dialogo.showAndWait();

		if( palabra.isPresent() && palabra.get().length() < 5) {
			
			Alert alert = new Alert(AlertType.INFORMATION);
			alert.setTitle("Palabra incorrecta");
			alert.setHeaderText("La palabra debe contener un minimo de 5 caracteres");
			alert.showAndWait();
			
		} else if( palabra.isPresent() ) {
			//Para evitar problemas futuros las palabras las guardamos todas en mayuscula
			listaPalabras.add(palabra.get().toUpperCase());
		}
